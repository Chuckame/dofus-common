...
public class LoopBackTimeStamp {
	...
	public long timeLapseInNanoSecond() {
		return this.recvTimeStamp - this.sendTimeStamp;
	}
	...
	public byte[] toByteArray() {
		...
		byte[] t1 = ByteBuffer.allocate(byteOfLong).putLong(this.sendTimeStamp).array();
		byte[] t2 = ByteBuffer.allocate(byteOfLong).putLong(this.recvTimeStamp).array();
	 ...
	 * @param content
	 */
	public void fromByteArray(final byte[] content) {
